...
function initializeAppStructure() {
  ...
  document.body.innerHTML = `
    ...
    <div class="actions">
      <button id="add-reward-btn" class="action-btn">➕ Add New Reward</button>
...
function setupEventListeners() {
  ...
  if (addRewardBtn) {
    addRewardBtn.addEventListener('click', showAddRewardModal);
  }
  ...
  try {
    ...
    let modalHTML = `
      <div class="goal-modal-header">
        ...
        <h2 class="goal-modal-title ${isEarned ? 'earned' : 'progress'}">${selectedReward}</h2>
    ...
    goals.forEach(goal => {
      modalHTML += `
        <button onclick="confirmGoalForReward('${selectedReward}', '${goal}')" 
                class="goal-button">
          🎯 ${goal}
        </button>
      `;
...
function confirmGoalForReward(rewardName, completedGoal) {
  ...
  const success = AppStorage.addActivityToReward(rewardName, activityEntry);
...
// Function to show activity history for a reward
function showActivityHistory(rewardName) {
  try {
    const data = AppStorage.getStoredData();
    ...
    let modalHTML = `
      <div class="activity-history-header">
        ...
        <h2 class="activity-history-title ${isEarned ? 'earned' : 'progress'}">${rewardName}</h2>
        ...
        modalHTML += `
          <div class="activity-item ${index % 2 === 0 ? 'even' : 'odd'} ${isEarned ? 'earned' : 'progress'}">
            ...
            <div class="activity-actions">
              <button onclick="deleteActivity('${rewardName}', ${index})" 
    ...
    modalHTML += `
      <div class="modal-actions">
        ${!isEarned ? `
          <button onclick="showGoalSelectionModal('${rewardName}'); closeActivityHistoryModal();" 
...
// Function to show add new reward modal
function showAddRewardModal() {
  // Create modal backdrop
  const modalBackdrop = document.createElement('div');
  modalBackdrop.id = 'add-reward-modal';
  modalBackdrop.className = 'modal-backdrop';
  ...
  let modalHTML = `
    <div class="add-reward-header">
      <div class="add-reward-icon">➕</div>
      <h2 class="add-reward-title">Add New Reward</h2>
      <div class="add-reward-description">Create a custom reward to work towards</div>
    </div>
    ...
    <div class="form-group">
      ...
      <input type="text" id="new-reward-name" placeholder="e.g., Weekend Trip, New Game, etc." 
             class="form-input">
    </div>

    <div class="button-group">
      <button onclick="confirmAddReward()" class="primary-button">
        ✅ Create Reward
      </button>
      <button onclick="closeAddRewardModal()" class="cancel-button">
        ❌ Cancel
      </button>
    </div>
  `;
  ...
  setTimeout(() => {
    const input = document.getElementById('new-reward-name');
...
function confirmAddReward() {
  const rewardNameInput = document.getElementById('new-reward-name');
  const rewardName = rewardNameInput.value.trim().toLowerCase();
  ...
  const data = AppStorage.getStoredData();
  if (!data) {
    showBriefMessage('❌ Error accessing app data', '#f44336');
    return;
  }

  // Check if reward already exists
  if (data.rewards[rewardName]) {
    showBriefMessage('❌ A reward with this name already exists', '#f44336');
    rewardNameInput.focus();
    return;
  }

  // Add new reward
  const success = AppStorage.addReward(rewardName);
  ...
  if (success) {
    ...
    showRewardAddedNotification(rewardName);
  } else {
    showBriefMessage('❌ Error creating reward. A reward with this name may already exist.', '#f44336');
  }
}
...
function loadEarnedRewards() {
  try {
    ...
    Object.keys(rewards).forEach(rewardName => {
...
function createRewardsWindow(earnedRewards, progressRewards, target) {
  ...
  if (earnedRewards.length > 0) {
    ...
    earnedRewards.forEach(reward => {
      rewardsHTML += `
        <div class="earned-reward-card" onclick="showActivityHistory('${reward.name}')">
  ...
  if (progressRewards.length > 0) {
    ...
    progressRewards.forEach(reward => {
      ...
      rewardsHTML += `
        <div class="progress-reward-card" onclick="showGoalSelectionModal('${reward.name}')">
          ...
          ${reward.activityCount > 0 ? `
            <button onclick="showActivityHistory('${reward.name}'); event.stopPropagation();" 
                    class="progress-history-button">📋</button>
          ` : ''}
          <button onclick="deleteReward('${reward.name}'); event.stopPropagation();" 
                  class="delete-reward-button progress"
                  title="Delete this reward">
            🗑️
          </button>
        </div>
...
function undoAddReward(rewardName) {
  ...
  const success = AppStorage.removeReward(rewardName);
...
// Function to show set target modal
...
// Target update notification system
